...
class MainWindow(QMainWindow):
    ...
    def refresh_map(self):
        ...
        self.update_address()  # Обновляем адрес
    ...
    def search_location(self):
        ...
        if response.status_code == 200:
            ...
            if geocode_data["response"]["GeoObjectCollection"]["featureMember"]:
                ...
                self.marker_ll = self.map_ll  # Сохраняем координаты метки
                self.map_zoom = 15
                self.update_address()  # Обновляем адрес
                self.refresh_map()
            else:
    ...
    def update_address(self):
        ...
        if response.status_code == 200:
            ...
            if geocode_data["response"]["GeoObjectCollection"]["featureMember"]:
                ...
                print(f"Адрес: {address}")  # Отладочное сообщение
                print(f"Почтовый индекс: {postal_code}")  # Отладочное сообщение
                ...
                print(f"Полный адрес: {full_address}")  # Отладочное сообщение
                self.address_label.setText(full_address)
            else:
                self.address_label.setText("Объект не найден")
                print("Объект не найден")  # Отладочное сообщение
        else:
            self.address_label.setText(f"Ошибка при запросе: {response.status_code}")
            print(f"Ошибка при запросе: {response.status_code}")  # Отладочное сообщение

    def reset_marker(self):
        self.marker_ll = None
        self.map_ll = self.initial_coords
        self.map_zoom = 10
        self.address_label.setText("Адрес найденного объекта")  # Сбрасываем адрес
        self.refresh_map()
